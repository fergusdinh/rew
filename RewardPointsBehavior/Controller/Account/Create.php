 ...
 */
namespace Lof\RewardPointsBehavior\Controller\Account;
...
class Create extends \Magento\Customer\Controller\AbstractAccount
{
    ...
    public function execute()
    {
        ...
        $resultRedirect = $this->resultRedirectFactory->create();
        if($referId){
            $resultRedirect->setPath('customer/account/create/refer/'.$referId);
        }elseif($referCode){
            $resultRedirect->setPath('customer/account/create/refercode/'.$referCode);
        }else{
            $resultRedirect->setPath('*/*');
        }
        return $resultRedirect;
