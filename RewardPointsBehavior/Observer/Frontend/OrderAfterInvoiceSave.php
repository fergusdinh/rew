        ...
        if ($this->rewardsConfig->isEnable()) {
            ...
            if(count($modelRefer)>0){
                ...
                if ($order && $customer_refer &&  $this->getConfig()->isEarnAfterInvoice()) {
                    $this->rewardsBehavior->setOrderId($order->getId())->processRule(Earning::BEHAVIOR_REFER_FRIEND, $customer_refer, $code, true, Transaction::STATE_COMPLETE, '', false, false, false, true);
                    $this->rewardsBehavior->setOrderId($order->getId())->processRule(Earning::BEHAVIOR_REFER_FRIEND, $customer_referred, $code, true, Transaction::STATE_COMPLETE, '', true, false, false, true);
                ...
                if($modelRefer->count()){
                    ...
                    if ($order && $customer_refer &&  $this->getConfig()->isEarnAfterInvoice()) {
                        ...
                        $this->rewardsBehavior->setOrderId($order->getId())->processAdvacedRule(Earning::BEHAVIOR_REFER_FRIEND, $customer_refer, $code, true, Transaction::STATE_COMPLETE, false, false, $number_orders);
                        $this->rewardsBehavior->setOrderId($order->getId())->processAdvacedRule(Earning::BEHAVIOR_REFER_FRIEND, $customer_referred, $code, true, Transaction::STATE_COMPLETE, true, $number_orders);
