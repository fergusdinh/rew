                ...
                if(count($modelRefer)>0){
                    ...
                    if ($customerReferId) {
                        ...
                        if ($order && $customer_refer && $this->getConfig()->isEarnAfterShipment()) {
                            ...
                            $this->rewardsBehavior->setOrderId($order->getId())->processAdvacedRule(Earning::BEHAVIOR_REFER_FRIEND, $customer_refer, $code, false, Transaction::STATE_COMPLETE, false, $number_orders);
                            $this->rewardsBehavior->setOrderId($order->getId())->processAdvacedRule(Earning::BEHAVIOR_REFER_FRIEND, $customer_referred, $code, false, Transaction::STATE_COMPLETE, true, $number_orders);
                            $model->setNumberOrders($number_orders);
                            $model->setTotalOrders($total_orders);
                            $model->save();
                        }
