    ...
    public function execute()
    {
        ...
        if ($data) {
            ...
            try {
                ...
                if($id && $storeId) {
                    $modelGlobalData = $this->rewardsData->setType(Spending::TYPE)->getGlobalRule($data['object_id']);
                }
                ...
                if ($id) {
                    $model->load($id);
                    if ($id != $model->getId()) {
                        throw new \Magento\Framework\Exception\LocalizedException(__('The wrong spending rate is specified.'));
                    }
                }
                ...
                if ($validateResult !== true) {
                    ...
                    $this->_redirect('rewardpoints/*/edit', [
                ...
                if (isset($data['monetary_step']) && (float)$data['monetary_step'] > 0) {
                    $data['monetary_step'] = (float) $data['monetary_step'];
                }

                if (isset($data['points_limit']) && (float)$data['points_limit'] > 0) {
                    $data['points_limit'] = (float) $data['points_limit'];
                }
                ...
                /** Update rate data by store view */
                if(!$storeId) {
                    $this->rewardsData->setType(Spending::TYPE)->updateRuleRelationShip($model, $useDefault);
                }
                ...
                if($this->getRequest()->getParam("duplicate")){
                    unset($data['rule_id']);
                    $rule1 = $this->_objectManager->create('Lof\RewardPoints\Model\Spending');
                    unset($data['object_id']);
                    $rule1->setData($data);
                    try{
                        $rule1->save();
                        if(!$storeId) {
                            $this->rewardsData->setType(Spending::TYPE)->updateRuleRelationShip($rule1);
                        }
                        $this->messageManager->addSuccess(__('You duplicated this rate.'));
                    } catch (\Magento\Framework\Exception\LocalizedException $e) {
                        $this->messageManager->addError($e->getMessage());
                    } catch (\RuntimeException $e) {
                        $this->messageManager->addError($e->getMessage());
                    } catch (\Exception $e) {
                        $this->messageManager->addException($e, __('Something went wrong while duplicating the rate.'));
                    }
                ...
                $this->rewardsLogger->addError(__('BUGS1: %1', $e->getMessage()));
