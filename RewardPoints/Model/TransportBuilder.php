<?php
/**
 * Landofcoder
 *
 * NOTICE OF LICENSE
 *
 * This source file is subject to the Landofcoder.com license that is
 * available through the world-wide-web at this URL:
 * https://landofcoder.com/license
 *
 * DISCLAIMER
 *
 * Do not edit or add to this file if you wish to upgrade this extension to newer
 * version in the future.
 *
 * @category   Landofcoder
 * @package    Lof_RewardPoints
 * @copyright  Copyright (c) 2016 Landofcoder (https://landofcoder.com/)
 * @license    https://landofcoder.com/LICENSE-1.0.html
 */

namespace Lof\RewardPoints\Model;

use Magento\Framework\App\TemplateTypesInterface;
use Magento\Framework\Mail\Message;
use Magento\Framework\Mail\MessageInterface;
use Magento\Framework\Mail\TransportInterfaceFactory;
use Magento\Framework\ObjectManagerInterface;
use Magento\Framework\Mail\MimeMessageInterface;

class TransportBuilder extends \Magento\Framework\Mail\Template\TransportBuilder
{
    /**
     * Template data
     *
     * @var array
     */
    protected $templateData = [];
    /**
     * @var \Zend_Mime_Part|string
     */
    protected $content;
    /**
     * @var string
     */
    protected $messageType = \Magento\Framework\Mail\MessageInterface::TYPE_HTML;
    /**
     * Set template data
     *
     * @param array $data
     * @return $this
     */
    /**
     * @var
     */
    protected $subject;

    /**
     * Get message content
     *
     * @return string
     */
    public function getMessageContent()
    {
//        return
//            $this->content instanceof MimeMessageInterface ?
//            $this->content->getMessage() :
//            $this->content;
        return
            $this->content instanceof \Zend_Mime_Part ?
                $this->content->getRawContent() :
                $this->content;
    }
     /**
     * Get message subject
     *
     * @return string
     */
    public function getMessageSubject()
    {
        return $this->subject;
    }

    /**
     * Prepare message
     *
     * @return $this
     */
    protected function prepareMessage()
    {
        parent::prepareMessage();

        $template = $this->getTemplate();
        if(method_exists($this->message, "getBodyText")){
            $this->content = $this->message->getBodyText();
        }elseif(method_exists($this->message, "getBody")){
            $this->content = $this->message->getBody();
        }
        $this->subject = (string)$this->message->getSubject();

        return $this;
    }

}
