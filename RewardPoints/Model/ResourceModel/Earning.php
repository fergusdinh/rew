...
class Earning extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
   /**
     * Store manager
     *
    ...
    protected function _beforeDelete(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $this->getConnection()->delete($this->getTable('lof_rewardpoints_earning_rule_relationships'), $condition);
    ...
    protected function _afterSave(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        // Rule Relationships
        $table = $this->getTable('lof_rewardpoints_earning_rule_relationships');
        $oldRelationshipRules = $this->lookupRelationshipRuleIds($object->getId());
        $where = ['rule_id = ?' => (int)$object->getId()];

        $connection = $this->getConnection();
        $select = $connection->select()->from($this->getTable('lof_rewardpoints_earning_rule_relationships'))->where(
            'rule_id = :rule_id'
            );
    ...
    public function lookupStoreIds($id)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable('lof_rewardpoints_earning_rule_store'),
            'store_id'
            )->where(
            'rule_id = :rule_id'
            );
    ...
    {
        $connection = $this->getConnection();
        $select = $connection->select()->from($this->getTable('lof_rewardpoints_earning_rule_customer_group'),'customer_group_id')->where(
            'rule_id = :rule_id'
            );
        $binds = [':rule_id' => (int)$id];
        return $connection->fetchCol($select, $binds);
    ...
    {
        $connection = $this->getConnection();
        $select = $connection->select()->from($this->getTable('lof_rewardpoints_earning_rule_relationships'),'rule_id')->where(
            'object_id = :object_id'
            );
