define([
	...
	], function ($, ko, ui, rewardsprice, quote, totals,getPaymentInformationAction) {
		'use strict';
		$.widget('lof.rewardsslider', {
			...
			_initSlider: function() {
				...
				this.slider = this.element.slider({
					step: self.options.sliderSettings.step > 0 ? self.options.sliderSettings.step : 1,
					min: self.options.sliderSettings.min,
					max: self.options.sliderSettings.max > 0 ? self.options.sliderSettings.max : 1000,
			...
			_initProgressBar: function (){
                ...
                var minMax = this.options.sliderSettings.max - this.options.sliderSettings.min;
                if (minMax) {
				    var style = 'style="width: ' + (((this.getValue() - this.options.sliderSettings.min) / minMax) * 100) + '%"';
                } else {
			...
			_onStopSlider: function() {
				...
				var discount = parseFloat((this.getValue() / this.options.sliderSettings.step) * this.options.sliderSettings.discount);
			...
			_setCurrentValue: function(val) {
				...
				var discount = parseFloat((val / this.options.sliderSettings.step) * this.options.sliderSettings.discount);
				this.updateDiscount(discount);
			},
			...
			ajaxSubmit: function() {
				if(this.options.ajaxUrl) {
					...
					$.ajax({
						...
						dataType: 'json',
						beforeSend: function () {
							if (!self.options.sliderSettings.message) {
						...
						// Notice: Update reward points data before total
						...
						if (rewardSegments) {
							...
							for (var i = 0; i < rewardSegments.length; i++) {
								for (var x = 0; x < totalSegments.length; x++) {
									...
									// Add tax value
									if (rewardSegments[i]['code'] == 'grand_total') {
										//totalSegments[x]['value'] = parseFloat(totalSegments[x]['value']) + parseFloat(taxTotal.value);
									}
								}
							}
						}
